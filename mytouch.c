...
//定义创建文件时的模式，此处用户，组，其他设置的权限都是可读可写
...
{
    int c;
    bool ok = true;    //初始化 ok?change_times?no_create：
    change_times = 0;
    no_create = false;
    //从执行命令中得到命令的选项，即以'-'开头的参数，目前只支出三种选型-a, -c, -m?
...
mytouch(const char *file)
{
	...
	if (change_times != (CH_ATIME | CH_MTIME))
	{
		...
		//如果change_times == CH_ATIME，即（-m），将对应的修改access time\
		timespec结构的tv_nesc设置为UTIME_OMIT；参考utimensat函数的用法
			newtime[0].tv_nsec = UTIME_OMIT;
		else
		//如果change_times = CH_MTIME,即 （-a），将对应的修改modify timme\
		的timespec结构的tv_nesc设置为UTIME_OMIT;参考utimensat函数的用法
